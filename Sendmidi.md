...
send OSC messages to port 46100 (p5), 57120(sc client), 12345(openframeworks)
...
Sendmidi {
	...
	*new { | server, p5addr, ofaddr, scaddr, chan = 0 |
		^super.new.init(server, p5addr, ofaddr, scaddr, chan);	//new.init
	}
	
	// start initialize	
	init { | argServer, argp5Addr, argofAddr, argscAddr, argChan = 0 |
